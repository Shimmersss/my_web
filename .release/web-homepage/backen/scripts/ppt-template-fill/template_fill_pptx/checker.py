...
def _image_region_lookup(library: dict[str, Any]) -> dict[tuple[int, str], dict[str, Any]]:
    lookup: dict[tuple[int, str], dict[str, Any]] = {}
    for slide in library.get("slides", []):
        slide_index = int(slide.get("slide_index", 0))
        for region in slide.get("image_regions", []):
            if region.get("region_id"):
                lookup[(slide_index, f"region_id:{region['region_id']}")] = region
    return lookup
...
def check_plan(library: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
    ...
    image_region_lookup = _image_region_lookup(library)
    ...
    for slide_index, slide in enumerate(plan.get("slides", []), start=1):
        ...
        for image_edit in image_edits:
            region_id = str(image_edit.get("region_id") or "")
            image_id = str(image_edit.get("image_id") or "")
            selector = f"region_id:{region_id}" if region_id else ""
            region = image_region_lookup.get((source_slide, selector)) if selector else None
            if not image_id:
                results.append(
                    {
                        "status": "ERROR",
                        ...
                        "source_slide": source_slide,
                        "selector": selector,
                        "message": "image edit requires image_id",
                    }
                )
                summary["error"] += 1
                continue
                ...
                results.append(
                    {
                        ...
                        "source_slide": source_slide,
                        "selector": selector,
                        "message": "image region target not found in slide library",
                    }
                )
                summary["error"] += 1
                continue
            if not bool(region.get("fillable")) or region.get("rejectReason"):
                results.append(
                    {
                        "status": "ERROR",
                        "plan_slide": slide_index,
                        ...
                        "rejectReason": region.get("rejectReason"),
                        "message": "image region is not fillable",
                    }
                )
                summary["error"] += 1
                continue
            ...
            results.append(
                {
                    ...
                    "region_id": region.get("region_id"),
                    "role": region.get("role"),
                    "message": "image edit region is fillable",
                }
            )
