  ...
  <section class="relationship-report">
    <header class="report-header">
      <div>
        ...
        <h1>{{ identity.titleId || identity.title || "月下会客厅 · 关系探索报告" }}</h1>
    ...
    <section class="chapter" aria-live="polite">
      <template v-if="activeChapter === 'identity'">
        ...
        <div class="tarot-preview"><div class="section-title"><span class="card-kicker">THREE CARDS</span><h3>三张牌，三个留给自己的问题</h3></div><div class="mini-cards"><article v-for="card in cards" :key="card.cardId"><div class="mini-card-art"><img :src="cardImage(card.cardId)" :alt="`${card.name}牌面`" @error="markImageFailed(card.cardId)" /><span v-if="failedImages[card.cardId]">✦</span></div><strong>{{ card.name }}</strong><small>{{ slotLabel(card.slot) }}</small></article></div><div class="tarot-reading-list"><article v-for="reading in tarotReadings" :key="reading.slot"><span class="card-kicker">{{ slotLabel(reading.slot) }}</span><p>{{ reading.interpretation }}</p></article></div></div>
      </template>
...
function downloadShareCard() {
  ...
  ctx.fillStyle = "#fff8ed"; ctx.font = "700 68px sans-serif"; wrapText(ctx, identity.value.titleId || identity.value.title || "月下会客厅 · 关系探索报告", 900).slice(0, 2).forEach((line, i) => ctx.fillText(line, 90, 270 + i * 88));
...
.tarot-preview { margin-top: 28px; padding-top: 22px; border-top: 1px solid #eadfcd; }.section-title h3 { margin: 0; color: var(--navy); }.mini-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-top: 16px; }.mini-cards article { text-align: center; }.mini-card-art { position: relative; aspect-ratio: 2 / 3; overflow: hidden; border-radius: 13px; background: linear-gradient(145deg, #182444, #805a73); }.mini-card-art img { width: 100%; height: 100%; object-fit: cover; }.mini-card-art span { position: absolute; inset: 0; display: grid; place-items: center; color: #f3dfad; font-size: 40px; }.mini-cards strong,.mini-cards small { display: block; margin-top: 8px; }.mini-cards small { color: var(--muted); }.tarot-reading-list { display: grid; gap: 10px; margin-top: 18px; }.tarot-reading-list article { padding: 16px 18px; border-radius: 14px; background: #fff; border: 1px solid #eee3d3; }.tarot-reading-list .card-kicker { margin: 0 0 6px; }.tarot-reading-list p { margin: 0; line-height: 1.8; }
