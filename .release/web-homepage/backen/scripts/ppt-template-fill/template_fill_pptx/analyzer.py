...
DECORATIVE_IMAGE_NAME_TOKENS = ("bg", "line", "shape")
DECORATIVE_IMAGE_NAME_TOKEN_CONTEXT = (
    "background",
    "decor",
    "decoration",
    "ornament",
...
def _image_region_fit(
    ...
    canvas_width = canvas.get("width") or 0
    canvas_height = canvas.get("height") or 0
    area = width * height
    canvas_area = canvas_width * canvas_height
    area_ratio = area / canvas_area if canvas_area > 0 else 0

    if _is_decorative_image_name(shape_name):
        return False, "decorative", "decorative_name"
    if width < 120 or height < 120 or area_ratio < 0.035:
        return False, "decorative", "too_small"
    if canvas_width > 0 and canvas_height > 0:
        near_origin = x <= canvas_width * 0.06 and y <= canvas_height * 0.06
        covers_canvas = width >= canvas_width * 0.78 and height >= canvas_height * 0.78
        if near_origin and covers_canvas and area_ratio >= 0.55:
            return False, "background", "background_like"
    if area_ratio >= 0.08:
        return True, "content_image", ""
    if width >= 260 and height >= 150 and area_ratio >= 0.045:
        return True, "small_content_image", ""
    return False, "decorative", "low_area_ratio"


def _image_page_allows_fill(page_type: str, region_role: str) -> bool:
    normalized = page_type.lower()
    if "cover" in normalized:
        return region_role in ("content_image", "small_content_image")
    if any(marker in normalized for marker in ("toc", "chapter", "ending")):
        return False
    return True


def _image_name_tokens(shape_name: str) -> list[str]:
    name = shape_name.lower()
    tokens = re.findall(r"[a-z]+|\d+|[\u4e00-\u9fff]+", name)
    expanded: list[str] = []
    for token in tokens:
        expanded.append(token)
        if re.search(r"[\u4e00-\u9fff]", token):
            expanded.extend(re.findall(r"[\u4e00-\u9fff]{1,2}", token))
    return expanded


def _is_decorative_image_name(shape_name: str) -> bool:
    name = shape_name.lower()
    if any(keyword in name for keyword in DECORATIVE_IMAGE_NAME_KEYWORDS):
        return True

    tokens = _image_name_tokens(shape_name)
    token_set = set(tokens)
    if not token_set.intersection(DECORATIVE_IMAGE_NAME_TOKENS):
        return False
    if token_set.intersection(DECORATIVE_IMAGE_NAME_TOKEN_CONTEXT):
        return True
    return any(token in DECORATIVE_IMAGE_NAME_TOKENS for token in tokens)
...
def _analyze_image_regions(
    ...
    for order, container in enumerate(_picture_containers(slide_root), start=1):
        ...
        area = (geometry.get("width") or 0) * (geometry.get("height") or 0)
        if area <= 0:
            continue
        fillable, region_role, reject_reason = _image_region_fit(shape_name, geometry, canvas)
        if fillable and not _image_page_allows_fill(page_type, region_role):
            fillable = False
            reject_reason = "page_type_not_fillable"
        regions.append(
            {
                "region_id": f"s{source_slide:02d}_img{shape_id}",
...
def analyze_pptx(pptx_path: Path) -> dict[str, Any]:
    ...
    with zipfile.ZipFile(pptx_path) as zf:
        ...
        for slide_ref in slide_refs:
            ...
            image_regions = _analyze_image_regions(slide_root, slide_ref.index, canvas, page_type)
            slides.append(
                {
