...
### fix: 翻译 PDF 参考文献按序号分条并悬挂缩进

**问题**：BabelDOC 0.6.3 会把部分论文的参考文献行合并成一个段落，译文中的多条文献挤在一起；自动换行也没有保留参考文献需要的悬挂缩进。

**改动**：
- 在 `babeldoc_runner.py` 安装窄范围兼容补丁，识别 `[1]`、`(1)`、`1.` 等连续序号并在翻译前把合并的参考文献拆成独立段落
- 对带年份、DOI、URL、期刊等文献特征的编号译文预留两个中文字符宽度，首行序号向左悬挂，续行从统一位置开始
- 文献特征判断独立到 `reference_layout.py`，避免普通步骤列表、正文编号被误套用参考文献排版
- 新增编号格式和误判保护单测

**验证**：
- `python3 -m unittest -v test_babeldoc_runner.py`：3 tests passed
- `python3 -m py_compile babeldoc_runner.py reference_layout.py test_babeldoc_runner.py`：通过
- `git diff --check`：通过
