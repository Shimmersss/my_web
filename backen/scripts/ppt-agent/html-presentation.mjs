...
.theme-terminal .kicker::before{content:"$ "}.theme-terminal .accent{height:2px}.theme-terminal .reveal h2{letter-spacing:-.02em}.theme-terminal .sources{color:var(--accent)}`,
...
function slideMarkup(slide, index, sourceById, imageById) {
  ...
  const sources = sourceIds.map(esc).join(', ');
  const sourceNotes = sourceIds.map(id => {
    const item = sourceById.get(id);
    return item ? `${id} | ${item.title || ''} | ${item.url || ''}` : id;
  }).map(esc).join('\n');
  const layout = ['cover', 'split', 'statement', 'evidence', 'comparison', 'timeline', 'quote', 'closing'].includes(slide.layout)
    ? slide.layout : (index % 3 === 0 ? 'split' : 'statement');
  const image = imageById.get(String(slide.imageId || ''));
  return `<section class="slide layout-${layout}" data-title="${esc(slide.title)}">
    ...
    ${layout === 'split' ? `<div class="split-panel"><span>${String(index + 1).padStart(2, '0')}</span><strong>${esc(slide.section || 'INSIGHT')}</strong></div>` : ''}
    ${sources ? `<small class="sources">${sources}</small>` : ''}
    <aside class="notes">[Sources]\n${sourceNotes}</aside>
  </section>`;
}

export async function createHtmlPresentation({ outputFile, plan, sources = [], sourceImages = [], templateKey, themeFile }) {
  const revealJs = await fs.readFile(path.resolve('node_modules/reveal.js/dist/reveal.js'), 'utf8');
  const revealCss = await fs.readFile(path.resolve('node_modules/reveal.js/dist/reveal.css'), 'utf8');
...
html,body,.reveal{background:var(--bg);color:var(--fg);font-family:Inter,"PingFang SC","Microsoft YaHei",sans-serif}
.reveal .slides{text-align:left}.reveal .slides section{box-sizing:border-box;padding:64px 72px}
...
.sources{position:absolute;left:72px;bottom:32px;font-size:13px;opacity:.58}.slide-number{color:var(--accent)!important}
.layout-cover{display:flex!important;flex-direction:column;justify-content:center}.layout-cover .accent{width:160px}
...
.split-panel{position:absolute;inset:0 0 0 63%;display:flex;flex-direction:column;align-items:center;justify-content:center;color:var(--fg);opacity:.88}.split-panel span{font-size:96px;font-weight:850;line-height:1;color:var(--accent)}.split-panel strong{max-width:70%;margin-top:24px;font-size:20px;letter-spacing:.12em;text-align:center}
...
.layout-timeline ul{display:flex;gap:18px;padding:0;list-style:none}.layout-timeline li{flex:1;padding:20px 18px;border-top:5px solid var(--accent);background:color-mix(in srgb,var(--surface) 78%,transparent)}
...
.evidence-image{position:absolute;right:54px;bottom:62px;width:38%;height:43%;margin:0}.evidence-image img{width:100%;height:100%;object-fit:contain;border-radius:16px}
.slide:has(.evidence-image){padding-right:45%!important}
${themeCss(style)}
@media(max-width:800px){.reveal .slides section{padding:42px}.reveal h2{font-size:44px}.headline{font-size:24px}.reveal ul{font-size:21px}.sources{left:42px}}
