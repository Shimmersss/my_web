...
if command -v nginx >/dev/null 2>&1; then
  ...
  sed "s#__INSTALL_DIR__#$INSTALL_DIR#g; s#__DOMAIN__#$DOMAIN#g" \
    "$CURRENT_DIR/deploy/nginx.conf.template" > "/etc/nginx/sites-available/$NGINX_SITE_NAME"
  if [[ -d /etc/nginx/sites-enabled ]]; then
    ln -sfn "/etc/nginx/sites-available/$NGINX_SITE_NAME" "/etc/nginx/sites-enabled/$NGINX_SITE_NAME"
  else
    warn "/etc/nginx/sites-enabled not found; enable /etc/nginx/sites-available/$NGINX_SITE_NAME manually."
  fi
  nginx -t
